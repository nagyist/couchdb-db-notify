...
exports.ensureDb = function (url) {
    return couchr.get(url, {}).otherwiseError(couchr.put(url, {}));
};

exports.withDb = function (url) {
    return ensureDb(url).map(couchr(url));
};
...
exports.handleUpdate = _.curry(function (target, update) {
    if (update.type === 'deleted') {
        target.del(update.db_name);
    }
    var db = url.resolve(target.url, update.db_name);
    var seq = exports.getSeq(db);
    var doc = seq.map(function (n) {
        return {
            _id: update.db_name,
            type: update.type,
            seq: n
    ...
    var updates = doc.flatMap(function (doc) {
        return target.put(update.db_name, doc);
    });
    return updates.errors(logError);
});
